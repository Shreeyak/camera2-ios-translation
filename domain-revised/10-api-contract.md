...
| `focusDistance` | double? | `[0.0, 1.0]`, units platform-defined | Lens focus distance, normalized (manual mode only) |
...
| `naturalTextureId` | int | Opaque identifier for the natural (unprocessed) preview surface |
| `activeCaptureResolution` | Size | The capture resolution in effect at session open |
| `activeCropRegion` | Rect | The crop region in effect at session open (x, y, width, height in capture-resolution pixels) |
| `streamPixelFormat` | string | Pixel format of delivered stream frames; always `"RGBA16F"` (4-channel half-float) |
...
| `focusDistance` | double? | Current lens focus distance, normalized `[0.0, 1.0]` (null during autofocus scan) |
...
**Note:** `focusDistance` is null whenever the autofocus system is actively scanning. It is only populated when focus is locked or passively settled.
...
### open
```
open(cameraId: String?, captureResolution: Size?, cropRegion: Rect?) → SessionCapabilities
```
Opens the camera and initializes the full capture pipeline. Returns session capabilities including the preview texture identifiers for the UI layer to render.
...
- `captureResolution`: The sensor capture resolution to request. Null defaults to the largest 4:3 resolution the device supports (typically ~4160×3120). Must be a resolution present in `supportedSizes`.
- `cropRegion`: The center-crop rectangle applied to the capture resolution before any processing. Null defaults to a 1600×1200 center crop. The crop region drives the resolution of the `natural`, `processed`, and `tracker` streams. The crop region may be changed at runtime via `setCropRegion`.

The natural and processed preview streams are always initialized; there is no flag to disable them. The method returns when the session is fully initialized. The returned texture identifiers can immediately be used by the UI layer.
...
### captureImage
```
captureImage(handle: Int, outputPath: String?) → String (file path)
```
Captures a still image from the current GPU-processed frame — the image exactly matches what the processed preview displays (including all color adjustments). Output format is always **8-bit TIFF** (lossless relative to the 8-bit output stage).

- `outputPath` null: saves to a platform-managed location (e.g., photo library or documents folder).
- `outputPath` non-null: saves directly to the specified path (must include filename with `.tiff` or `.tif` extension).

Returns the absolute saved file path. Only one capture may be in flight at a time; concurrent calls are rejected.
...
- Default bitrate: `TARGET_BITRATE_MBPS` (a platform-specific measured value; see `measurements/`)
- Default fps: 30
...
### setCropRegion
```
setCropRegion(handle: Int, cropRegion: Rect) → void
```
Adjusts the center-crop rectangle applied to the captured sensor frame. Takes effect on the next committed frame. The crop region must lie within the current capture resolution; invalid regions are rejected with `INVALID_STATE`. Brief frame drops may occur during GPU pipeline reconfiguration.

[audit: 03-capture-pipeline.md §Crop, 10-api-contract.md §open]
...
The pipeline exposes a registration surface through which any component can subscribe to, and unsubscribe from, a named output stream. The defined streams are `natural`, `processed`, and `tracker`. Registration is per-subscription; unsubscribing affects only the caller's subscription and must not affect delivery to other subscribers. Each stream has a **latest-only** delivery policy; subscribers requiring every frame (e.g., a recording sink) must implement their own buffering. The location of the registry, the delivery mechanism (callback, channel, queue), and the language of the subscribing component are platform concerns.

Delivered frames are in **RGBA16F** format (4 half-float channels, 8 bytes per pixel). Frame data is valid only for the duration of the delivery callback or until the subscriber explicitly retains it. Subscribers that hold a frame beyond the callback scope must copy the pixel data.
