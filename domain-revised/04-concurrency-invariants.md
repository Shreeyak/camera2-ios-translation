...
The native C++ pipeline object is accessed by two concurrent paths: the camera controller (teardown) and capture requests (captureImage). A teardown that zeroes the pointer while a capture is reading it would produce a use-after-free.
...
The "capture in flight" flag must be an atomic compare-and-swap operation. A race between two concurrent `captureImage()` calls must result in exactly one capture proceeding and one being rejected — never two captures running simultaneously.
...
## Invariant 12: Watchdog Callbacks Observe Only Their Originating Session

A watchdog callback (GPU-level or capture-result-level) that fires after its session has been torn down is a no-op. Callbacks must not read or mutate any state belonging to a session other than the one that was current when the watchdog was armed.

**Required guarantee:** Each watchdog carries a session reference or session token captured at arm time. Before any callback action, the watchdog confirms the session is still the active session; if not, it exits immediately without error.

[audit: 08-error-recovery.md §Stall Watchdog, 02-frame-delivery.md §Watchdog Lifecycle]
