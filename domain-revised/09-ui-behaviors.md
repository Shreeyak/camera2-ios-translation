...
Both preview streams are always active when the session is in `"streaming"` state. The split-screen is always shown with both halves present.
...
`focusDistance` is null during autofocus scans; the UI should show a visual indicator (e.g., scanning animation) rather than displaying a numeric value when this field is null.
...
The application runs in **landscape-right** orientation (device USB port on the left after rotation). All preview and UI elements are laid out for this fixed orientation. The system does not dynamically reflow for portrait or landscape-left.
