...
|---|---|
| Mode values | `"auto"` \| `"manual"` |
| Manual field | `focusDistanceDiopters`: lens focus distance in diopters |
| Auto behavior | Continuous autofocus |
| Focus at infinity | `0.0` diopters |
| Default | `"auto"` |

In `"auto"` mode, the autofocus system continuously adjusts focus. In `"manual"` mode, the lens is set to the specified diopter value and held fixed.

**`focusDistanceDiopters` during autofocus**: When autofocus is actively scanning, the `focusDistanceDiopters` field in `FrameResult` callbacks is null. A numeric value is only reported when focus is locked (settled). See `09-ui-behaviors.md` for UI implications.
...
| Property | Value |
|---|---|
| Field | `noiseReductionMode`: integer |
| Values | Device-dependent integer passthrough |

The noise reduction integer value is passed directly to the camera hardware without interpretation. Valid values are device-specific. The application layer is responsible for providing valid values.

**Note for downstream architect**: See `11-what-not-to-port.md` — the integer encoding is platform-specific. The target platform equivalent (if any) will have different values and controls.
...
| Property | Value |
|---|---|
| Field | `edgeMode`: integer |
| Values | Device-dependent integer passthrough |

Same passthrough semantics as noise reduction mode. The application layer provides integer values that the system forwards to the camera hardware.

**Note for downstream architect**: See `11-what-not-to-port.md`.
...
The system monitors auto-exposure convergence. If the auto-exposure system remains in a "searching" state for more than **5000ms** continuously:
...
## Capture Mode for Repeating vs. Still Requests

The system uses different optimization settings for the repeating (preview/recording) request vs. the one-shot still capture request:
...
- **Still capture** (hardware ISP path only): A one-shot request with ISP settings optimized for still image quality.
