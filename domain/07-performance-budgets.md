...
| Budget | Value | Notes |
|---|---|---|
| GPU fence wait | 8ms | Maximum wait for GPU readback completion; falls back to full GPU flush on expiry |
...
The GPU fence wait budget of 8ms is chosen to prevent stalling the render loop while allowing reasonable GPU completion time. Exceeding this budget forces a blocking GPU flush, which may cause a visible frame drop. Repeated fence timeouts indicate GPU performance degradation.
...
| Threshold | Value |
|---|---|
| AE convergence timeout | 5000ms |

If auto-exposure remains in a searching state for more than 5 seconds, a non-fatal `AE_CONVERGENCE_TIMEOUT` notification is emitted. This is informational only — no recovery action is taken.
...
**Frame buffer size**: `width × height × 4 bytes` (RGBA8888).

At the default 4:3 maximum resolution of the target device (4160×3120 = 12,979,200 pixels), the full-resolution frame buffer is approximately **49.5 MB** per frame.

Two readback buffers alternate (double-buffering), so GPU readback buffer allocation is approximately **99 MB** at maximum resolution.

The tracker frame buffer (480px height, aspect-preserving width) is approximately **0.7 MB** per frame at the 4160×3120 source resolution.

All registered C++ consumers receive a reference to the same frame allocation — no per-consumer copies.
