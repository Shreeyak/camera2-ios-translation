...
Frame rate target (30fps). Input format (planar YUV 4:2:0, largest 4:3 resolution). Eight-step GPU pipeline sequence. Asynchronous double-buffered readback with 8ms fence budget. Four parallel output streams (processed full-res, tracker fixed 480px, natural, encoder). Preview surface rebind on swap failure. Drop-on-busy consumer dispatch semantics. Per-frame metadata (9 fields). Dual stall watchdog (3s GPU-level, 5s capture-result-level).
...
Frame rate (30fps target, 15fps degradation threshold). GPU fence budget (8ms). Frame delivery lag (1 frame). Stall thresholds (3s GPU, 5s result). Recovery backoff schedule. HAL error threshold (5). AE convergence (5s). Frame buffer sizes (49.5 MB at 4160×3120). Consumer processing semantics. Resolution resize timeout (5s). Drain timeout (5s). Preview surface failure threshold (3 swaps). Metadata array sizes.

### `08-capture-and-recording.md`
Two still capture paths: hardware ISP (no GPU processing, JPEG) and GPU-processed (matches preview, JPEG/PNG). One-capture-at-a-time concurrency guard. EXIF metadata requirements. Video encoding requirements: GPU-to-encoder zero-copy, HEVC preferred / H.264 fallback, 50Mbps/30fps defaults. Recording state machine. Start/stop flows. Drain timeout behavior. Recording stops automatically on pause.
...
### `10-api-contract.md`
7 data types (CameraSettings, ProcessingParameters, SessionCapabilities, SessionState, ErrorCode, Error, FrameResult, RgbSample). 16 host methods (open, close, pause, resume, backgroundSuspend, backgroundResume, updateSettings, setProcessingParameters, getPersistedProcessingParameters, sampleCenterPatch, captureNaturalPicture, captureImage, startRecording, stopRecording, setResolution, getNativePipelineHandle). 4 callbacks (onStateChanged, onError, onFrameResult, onRecordingStateChanged). C++ consumer registration API.
