...
The capture pipeline uses named request templates from the Android camera hardware abstraction layer to initialize capture parameters for different modes. The template choice affects ISP tuning (preview vs. recording vs. still capture).

**Why not to port:** The template identifiers are specific to the Android camera API. The behavioral requirement — that capture parameters are tuned differently for preview, recording, and still capture modes — is a domain requirement (see `03-camera-control.md`). [audit: 09-camera-controls.md §capture-mode-templates]

---

## Android-Specific Integer Passthrough for Noise Reduction and Edge Enhancement

The API exposes noise reduction mode and edge enhancement mode as raw integer values that are passed directly to the camera hardware abstraction layer without validation or interpretation.

**Why not to port:** The integer values map to noise-reduction and edge-mode constants in the Android camera API. On the target platform, the equivalent capability (if available) will have different controls and value ranges. The behavioral requirement — that the system exposes passthrough control over sensor noise reduction and edge enhancement when supported — is a domain requirement, but the specific integer encoding is not. [audit: 04-pigeon-api.md §CamSettings, 09-camera-controls.md]
...
The video encoder output is drained by a dedicated loop on a separate execution context that polls the encoder output queue with a 10ms timeout and writes encoded packets to the container muxer.
...
## HEVC/H.264 Codec Selection Logic

The video encoder prefers HEVC (`video/hevc`) and falls back to H.264 (`video/avc`) based on device capability query.

**Why not to port:** The codec name strings are Android video codec identifiers. The behavioral requirement — that the system prefers a more efficient codec when available (HEVC preferred, H.264 fallback) — is a domain requirement. The specific codec query mechanism is platform-specific. [audit: 10-capture-recording.md §Codec Selection]
