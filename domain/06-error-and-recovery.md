...
1. **Check terminal state**: If the session is already in terminal error state, exit immediately (no additional actions).
2. **Check background suspension**: If the application is suspended (fully invisible), transition to closed state silently and exit.
3. **Transition state to `"recovering"`**: Emit `"recovering"` state notification to the application layer.
4. **Emit error event**: Notify the application layer with the error code and `isFatal: false`.
5. **Check retry budget**: If the retry count has reached the maximum (5 retries), transition to fatal error.
6. **Cancel duplicate retries**: If a retry is already pending, cancel it before scheduling the new one.
7. **Schedule retry**: Post a retry after the backoff delay for the current retry count.
...
- Close still-capture reader.
- Stop GPU pipeline.
- Release native C++ pipeline (pointer zeroed under lock).
