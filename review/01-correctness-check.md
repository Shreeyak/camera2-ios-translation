...
**Category A verdict: YELLOW** — 10 PASS, 2 PARTIAL (A-03 diopter deviation documented-but-real; A-08 EXIF schema deferred). No FAIL.
...
| C-03 | **Correct OpenCV color conversion constant** for Metal output format | **FAIL** | `design/04-opencv-integration.md` line 242: `cv::cvtColor(rgba, gray, cv::COLOR_RGBA2GRAY)`. Metal outputs `BGRA8Unorm` (design decision D-14, `03-metal-pipeline.md` texture spec table). The correct constant is `cv::COLOR_BGRA2GRAY`. Using `COLOR_RGBA2GRAY` swaps the R and B channel weights in the luminance formula (ITU-R: 0.299R + 0.587G + 0.114B), producing incorrect grayscale where blue-dominant and red-dominant regions appear inverted in intensity. Edge detection will run but on corrupted grayscale — contours will be shifted, missed, or hallucinated relative to actual edges. |
...
**Category C verdict: RED** — 6/7 PASS, 1 FAIL (C-03: `cv::COLOR_RGBA2GRAY` must be `cv::COLOR_BGRA2GRAY`). This is a correctness bug that will produce silently wrong edge detection output.
