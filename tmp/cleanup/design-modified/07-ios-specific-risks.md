...
|---|---|---|---|---|---|
| R-01 | Thermal throttling causes preview frame drops or session suspension | 1a, 4 | High | High | `ProcessInfo.ThermalState` monitoring installed in Phase 1a; v1: banner-only warning when thermal state reaches `.serious` or `.critical` — no frame-rate reduction, no pipeline degradation, no session teardown. Full degradation response deferred to a later phase. |
...
| R-07 | ~~OpenCV iOS headers incompatible with Swift-C++ direct interop~~ **NO LONGER A RISK under D-16.** | 3 | — | — | Swift never imports `<opencv2/*>`. OpenCV is a private implementation detail of `Sources/ImagingCore/src/*.cpp` — banned from all public headers and from all Swift code. The Clang module importer only sees POD, `std::span`, and enum classes, which Swift 6.2+ handles cleanly. See D-16. |
| R-08 | OpenCV xcframework binary size increase (~80–120 MB added) | 3 | High | Medium | Distributed as SwiftPM `binaryTarget` wrapping `opencv2.xcframework` (see D-19). SwiftPM auto-selects device/simulator slices, so no `EXCLUDED_ARCHS` hack is needed. For release-build size reduction, consider switching to a source build via CMake that links only `core` + `imgproc` — this would require adding a CMake-driven build step to the package but keeps the public `ImagingCore` interface unchanged. |
...
| R-15 | Preview surface rebind race: `MTKView.currentDrawable` nil during rebind | 2, 4 | Low | Low | `MetalRenderer.draw(_:)` guards on nil drawable; consecutive failure counter resets after successful rebind; rebind runs inside `CaptureActor` (serialized) |
...
| R-18 | `PixelSink::onFrame()` called from Swift; uncaught C++ exception would abort the process | 3 | Low | **Critical** | **Superseded mitigation:** the old design relied on ObjC++ `@try/@catch`; under D-16 there is no ObjC++ layer. The new mitigation is three-layer: (1) every public method on the `EdgeDetectorFacade` is `noexcept` and wraps its body in `try { ... } catch (const cv::Exception&) { return ErrorCode::OpenCVFailure; } catch (const std::exception&) { return ErrorCode::InternalFailure; } catch (...) { return ErrorCode::InternalFailure; }`; (2) a dedicated poison test in `ImagingCoreTests` feeds malformed input to every public method and asserts the return is an `ErrorCode`, not a crash — runs via `swift test` on the macOS host; (3) `Interop` has no defensive try/catch, so any facade violation crashes on the C++ side (attributable) rather than on the Swift runtime (confusing). See `02-concurrency.md §Exception Discipline at the C++ Boundary`. |
...
| R-28 | ~~Linux CI runner drift~~ — **NOT APPLICABLE.** Linux CI is out of scope. `ImagingCore` is tested on the macOS host via `swift test` only. If headless Linux builds are introduced later, this risk should be revisited. | — | — | — | — |
| R-29 | Half-float precision in the Canny path — `cv::Canny` requires `CV_8UC1`, so `EdgeDetector` converts half-float → 8-bit internally. If the conversion scale is wrong (e.g. clamp range, gamma assumption), edges silently degrade without any runtime error. | 3 | Medium | Low | Golden-fixture test in `ImagingCoreTests` that uses a pre-recorded frame (`kCVPixelFormatType_Lossless_420YpCbCr8BiPlanarFullRange`) with known-good contour count at specific Canny thresholds. The test fails if the contour count drifts beyond a ±5% tolerance. Runs via `swift test`. |
| R-30 | Inspector Window (iPadOS 26) scene-restoration edge cases — the secondary `Window("Inspector", id: "inspector")` can be closed by the user, moved to a separate Stage Manager space, or become orphaned after a scene-restoration event. If `CameraViewModel` holds a strong reference to the inspector's state while the window is closed, memory is leaked; if it holds a weak reference, re-opening the window loses history. | 5 | Low | Low | `CameraViewModel` owns the capture history (persisted via `@AppStorage` / file). `InspectorView` reads from the view model via `@Environment`. Closing the window discards view-local state only; re-opening reads the history fresh from the view model. Histogram state is per-window and derived from the current Metal readback buffer — no persistence needed. Test: open → capture → close Inspector → re-open, verify history intact. |
...
|---|---|---|
| Capture session configuration rejected (`CONFIGURATION_FAILED`) | `CaptureActor.configureSession()` | `AVCaptureSession` `AVCaptureSessionRuntimeErrorNotification` with `AVError.sessionConfigurationChanged`; caught → non-fatal recovery |
| Camera device disconnected (`CAMERA_DISCONNECTED`) | `CaptureActor` + `SystemPressureMonitor` | `AVCaptureSessionRuntimeErrorNotification` with `AVError.deviceIsNotAvailableInBackground` or `.videoDeviceNotAvailable` |
| Camera device access error (`CAMERA_ACCESS_ERROR`) | `CaptureActor.open()` error handler | `AVCaptureDevice` throws `AVError`; catch → non-fatal; 5-retry backoff |
| Platform security bug after keyguard dismiss | `CaptureActor` interrupt handler | On iOS: Secure Enclave re-authentication; no exact equivalent to Android keyguard bug. System reports `AVCaptureSessionInterruptionReasonVideoDeviceNotAvailableInBackground` — treated as non-fatal `CAMERA_ACCESS_ERROR` |
| 5 consecutive hardware-level frame failures (`CAPTURE_FAILURE`) | `CaptureActor.captureOutput(_:didDrop:from:)` | `AVCaptureVideoDataOutput` `captureOutput(_:didDrop:from:)` counts consecutive drops; at 5 → `handleNonFatalError(.captureFailure)` |
...
| EOS drain timeout during recording stop (`RECORDING_TRUNCATED`) | `RecordingActor.stopRecording()` | `AVAssetWriter.finishWriting` with 5s `Task.sleep` deadline; if timeout → `AVAssetWriter.cancelWriting` + emit `RECORDING_TRUNCATED` + return partial URI |
| Camera in use by another app (`CAMERA_IN_USE`) | `CaptureActor` + `CameraAvailabilityMonitor` (Phase 6) | `AVCaptureSessionInterruptionReasonVideoDeviceNotAvailableWithMultipleForegroundApps` → fatal error; KVO on `AVCaptureDevice` for availability → self-heal |
| Max retries exceeded (`MAX_RETRIES_EXCEEDED`) | `CaptureActor.handleNonFatalError()` | After 5th failed retry → `handleFatalError(.maxRetriesExceeded)` → full teardown + `.error` state |
...
| Video encoder init failure (`RECORDING_START_FAILED`) | `RecordingActor.startRecording()` | `AVAssetWriter` init throws → fatal `RECORDING_START_FAILED` |
| Recording pipeline failure mid-session (`RECORDING_FAILED`) | `RecordingActor` error observer | `AVAssetWriter.status == .failed` observed in drain loop → fatal `RECORDING_FAILED` |
...
| Open while session active | `CaptureActor.open()` guard | `guard sessionState == .closed || sessionState == .permissionDenied else { throw CameraError(.invalidState) }` |
| HAL error threshold (5 consecutive) | `CaptureActor.captureOutput(_:didDrop:from:)` | `consecutiveDropCount` incremented; reset on successful frame; at 5 → non-fatal recovery |
...
| Gradle / NDK / CMake build configuration | Not used — Xcode + SwiftPM + `binaryTarget` wrapping `opencv2.xcframework`. `ImagingCore` C++ tests run via `swift test` on the macOS host (no separate CMake build). A headless CMake/Linux build could be added later if non-Apple portability becomes a goal, but is not in scope for 2026-04-16. |
