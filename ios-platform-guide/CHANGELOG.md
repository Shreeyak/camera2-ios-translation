...
## 2026-04-20 — swift-architect review amendments

Source: swift-architect subagent review of `design/` (the generated iOS architecture
output, produced by the pre-existing DESIGN stage). `design/` is NOT patched by this
change; it will be regenerated by the Agent 3 pipeline.

### New ADRs
- ADR-30 — AVCaptureSession lifecycle via async with timeout (`04-avfoundation.md`)
- ADR-31 — Swift-subclassing C++ abstract class is unproven; spike first (`05-interop.md`)
- ADR-32 — CaptureDeviceProviding DI seam for testability (`01-architecture.md`)
- ADR-33 — Testing strategy: Swift Testing + XCTest split (`07-code-style.md`)

### New gotchas
- G-31 — Raw `Task { @MainActor }` on hot-path delegates is forbidden for UI updates
- G-32 — `MTLBlitCommandEncoder.copy` does not format-convert
- G-33 — Label `MTLCommandBuffer`s; use `pushDebugGroup` for Xcode capture
- G-34 — `IOSurfaceGetBytesPerRow` vs `width * bpp` — stride always required for `cv::Mat` views

### Updates
- ADR-02 / ADR-07 (`02-concurrency.md`): literal Swift 6 delegate class declaration with explicit `final`, `NSObject`, `@unchecked Sendable`, `nonisolated` attributes.
- ADR-11 (`05-interop.md`): module-map discipline promoted from mitigation to day-one requirement; `module.modulemap` example added.
- ADR-14 (`04-avfoundation.md`): strict-concurrency requirements for KVO→AsyncStream (`[weak self]`, `onTermination` cleanup, deterministic close in owning actor).
- ADR-16 (`04-avfoundation.md`): background-during-recording resolved — expiration handler always calls `cancelWriting`; `finishWriting` only pre-background or on foreground resume.
- ADR-20 (`03-metal.md`): demoted dynamic `.private`/`.shared` rotation to a graduation path; v1 default is always-`.shared` on Apple Silicon with Instruments-driven upgrade.
- G-08 (`06-gotchas.md`): cross-reference to ADR-16's background-during-recording rule.
- G-30 (`06-gotchas.md`): at-most-once worked example added.
